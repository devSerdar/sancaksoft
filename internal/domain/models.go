...
// CreatePaymentRequest is the DTO for creating a new payment
...
type CustomerBalanceDetail struct {
	...
	CurrentBalance decimal.Decimal `json:"current_balance"` // Sales - Returns - Payments = Borç
}

// CustomerLedgerDetailEntry represents a single ledger entry with full details
type CustomerLedgerDetailEntry struct {
	ID            uuid.UUID       `json:"id"`
	Date          time.Time       `json:"date"`
	Type          string          `json:"type"` // SALE, RETURN, PAYMENT
	Description   string          `json:"description"`
	Debit         decimal.Decimal `json:"debit"`  // Borç (satış)
	Credit        decimal.Decimal `json:"credit"` // Alacak (ödeme, iade)
